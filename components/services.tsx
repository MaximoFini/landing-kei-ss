...
function ServiceCard({ service, index }: { service: Service; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Light wash — grows from a point at the top on hover, collapses back into it on leave */}
      <div
        className="absolute inset-0 [clip-path:circle(0%_at_50%_0%)] group-hover:[clip-path:circle(150%_at_50%_0%)] transition-[clip-path] duration-700 ease-out pointer-events-none"
        style={{
          background:
      ...
      {/* Lamp — a cord drops down from the top edge with a bulb at its tip, lighting the card */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 z-20 h-0 opacity-0 group-hover:h-12 sm:group-hover:h-14 group-hover:opacity-100 transition-[height,opacity] duration-500 ease-out overflow-visible pointer-events-none flex flex-col items-center">
