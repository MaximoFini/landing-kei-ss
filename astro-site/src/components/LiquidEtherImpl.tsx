// @ts-nocheck
/*
 * Ported from components/LiquidEther.jsx — imperative Three.js WebGL fluid sim.
 * Only the framework shell changed: hooks come from `preact/hooks` instead of
 * `react`. Every bit of sim logic (GLSL shaders, FBO ping-pong, Simulation,
 * AutoDriver, Mouse takeover/click-burst, IntersectionObserver + ResizeObserver
 * + visibilitychange lifecycle) is a verbatim copy of the source.
 */
  ...
  useEffect(() => {
    ...
    class MouseClass {
      constructor() {
        ...
        // Impulso de clic: una "salpicadura" corta y potente que se inyecta
        // encima del arrastre normal, sin depender de que el mouse se mueva.
        this.clickBurst = {
          active: false,
          start: 0,
          duration: 260,
        ...
        this._onMouseDown = this.onDocumentMouseDown.bind(this);
      ...
      init(container) {
        ...
        this.listenerTarget.addEventListener('mousedown', this._onMouseDown);
      ...
      dispose() {
        if (this.listenerTarget) {
          ...
          this.listenerTarget.removeEventListener('mousedown', this._onMouseDown);
      ...
      onDocumentMouseDown(event) {
        if (!this.updateHoverState(event.clientX, event.clientY)) return;
        if (this.onInteract) this.onInteract();
        this.triggerClickBurst(event.clientX, event.clientY);
      }
      triggerClickBurst(clientX, clientY) {
        ...
        this.clickBurst.point.set(nx * 2 - 1, -(ny * 2 - 1));
        const angle = Math.random() * Math.PI * 2;
        this.clickBurst.dir.set(Math.cos(angle), Math.sin(angle));
        this.clickBurst.start = performance.now();
        this.clickBurst.active = true;
        this.takeoverActive = false;
        this.isAutoActive = false;
        this.hasUserControl = true;
      }
      onDocumentTouchStart(event) {
        ...
        if (this.onInteract) this.onInteract();
        this.triggerClickBurst(t.clientX, t.clientY);
      ...
      update() {
        if (this.clickBurst.active) {
          const t = (performance.now() - this.clickBurst.start) / this.clickBurst.duration;
          if (t >= 1) {
            this.clickBurst.active = false;
...
`;
    ...
    float velLen(vec2 sampleUv){
      return clamp(length(texture2D(velocity, sampleUv).xy), 0.0, 1.0);
    }
    ...
    // Vidrio líquido, sin derivadas (fwidth rompía la compilación en algunos
    // drivers): se compara con los 4 vecinos para encontrar el "canto" de la
    // onda -- el filo curvo que atrapa luz en un vidrio de verdad.
    float l_l = velLen(uv - vec2(px.x * 2.0, 0.0));
    float l_r = velLen(uv + vec2(px.x * 2.0, 0.0));
    float l_u = velLen(uv + vec2(0.0, px.y * 2.0));
    float l_d = velLen(uv - vec2(0.0, px.y * 2.0));
    float edge = clamp((abs(l_r - l_l) + abs(l_u - l_d)) * 6.0, 0.0, 1.0);

    // Dos capas de brillo especular: una angosta y potente en las crestas
    // (el destello) y otra amplia y tenue que da la sensación de superficie
    // pulida, más el realce de canto -- así se lee como vidrio y no como
    // tinta plana.
    float specTight = pow(lenv, 6.0);
    float specSoft = pow(lenv, 1.6);
    vec3 glassC = c + specTight * 0.32 + specSoft * 0.06 + edge * 0.16;

    vec3 outRGB = mix(bgColor.rgb, glassC, lenv);
